...
safe_upper <- function(x) ifelse(is.na(x), NA_character_, str_to_upper(x))

find_expression_sample_columns <- function(df) {
  frac_numeric <- sapply(df, function(col) suppressWarnings(mean(!is.na(as.numeric(col)))))
...
infer_groups <- function(sdrf, sample_col) {
  ...
  temperature_group <- ifelse(
    ...
    ifelse(str_detect(collapsed, "control|thermoneutral|warm|24c|baseline|room temp"), "CONTROL", "UNKNOWN")
  )
...
metadata_sanity_checks <- function(sdrf, idf = NULL, out_dir, log_lines) {
  log_lines <- c(
    log_lines,
  ...
  sdrf_col_summary <- tibble(
    ...
    first_examples = sapply(sdrf, function(x) paste(head(unique(x[!is.na(x) & x != ""]), 5), collapse = " | "))
  )
  ...
  if (length(tissue_cols) > 0) {
    sdrf |> select(all_of(tissue_cols)) |> distinct() |> head(30) |>
      write_csv(file.path(out_dir, "csv", "sdrf_tissue_related_preview.csv"))
  }
  if (length(temp_cols) > 0) {
    sdrf |> select(all_of(temp_cols)) |> distinct() |> head(30) |>
      write_csv(file.path(out_dir, "csv", "sdrf_temperature_related_preview.csv"))
  }
...
run_limma <- function(bat, out_dir, log_lines) {
  if (!requireNamespace("limma", quietly = TRUE)) {
    log_lines <- c(log_lines, "WARNING: limma not installed; skipping limma differential analysis.")
    return(log_lines)
  }

  bat_gene <- bat |>
    filter(!is.na(gene_symbol), gene_symbol != "") |>
    group_by(gene_symbol, sample_id, temperature_group) |>
    summarise(expression = mean(expression, na.rm = TRUE), .groups = "drop")

  if (nrow(bat_gene) == 0) {
    log_lines <- c(log_lines, "WARNING: No gene-level BAT data for limma.")
    return(log_lines)
  }

  samples <- bat_gene |>
    distinct(sample_id, temperature_group) |>
    mutate(group = factor(safe_upper(temperature_group), levels = c("CONTROL", "COLD")))

  if (length(unique(samples$group)) < 2) {
    log_lines <- c(log_lines, "WARNING: Need both CONTROL and COLD groups for limma.")
    return(log_lines)
  }

  expr_wide <- bat_gene |>
    select(gene_symbol, sample_id, expression) |>
    pivot_wider(names_from = sample_id, values_from = expression)

  mat <- as.matrix(expr_wide[, -1, drop = FALSE])
  rownames(mat) <- expr_wide$gene_symbol

  sample_order <- colnames(mat)
  sample_df <- samples |> filter(sample_id %in% sample_order) |> arrange(match(sample_id, sample_order))

  design <- model.matrix(~ group, data = sample_df)
  fit <- limma::lmFit(mat, design)
  fit <- limma::eBayes(fit)
  tt <- limma::topTable(fit, coef = "groupCOLD", number = Inf, sort.by = "P") |>
    tibble::rownames_to_column("gene_symbol")

  write_csv(tt, file.path(out_dir, "csv", "bat_cold_vs_control_limma_all_genes.csv"))

  cldn1_limma <- tt |> filter(safe_upper(gene_symbol) == "CLDN1")
  write_csv(cldn1_limma, file.path(out_dir, "csv", "cldn1_limma_stats.csv"))

  log_lines <- c(log_lines, sprintf("limma completed on %d genes.", nrow(tt)))
  log_lines
}
...
run_analysis <- function(opt) {
  if (is.null(opt$sdrf)) stop("--sdrf is required.")
  ...
  idf <- if (!is.null(opt$idf)) read_tabular(opt$idf, has_header = FALSE) else NULL

  log_lines <- metadata_sanity_checks(sdrf = sdrf, idf = idf, out_dir = out_dir, log_lines = log_lines)

  if (isTRUE(opt$`metadata-only`) || is.null(opt$expression)) {
    log_lines <- c(log_lines, "Metadata-only mode: expression analysis skipped.",
    ...
    message("Metadata checks complete. See: ", file.path(out_dir, "logs", "run_summary.txt"))
    return(invisible(list(mode = "metadata_only", out_dir = out_dir)))
  }
  ...
  if (length(sample_cols) < 2) stop("Could not identify sample columns in expression matrix. Check format/delimiter.")
  ...
  if (is.na(sdrf_sample_col) && is.null(opt$`sample-map`)) stop("No SDRF/expression sample overlap; provide --sample-map.")
  if (!is.na(sdrf_sample_col)) log_lines <- c(log_lines, sprintf("Detected SDRF sample ID column: %s", sdrf_sample_col))

  if (!is.null(opt$`sample-map`)) {
    sample_map <- read_tabular(opt$`sample-map`)
    required <- c("sample_id", "tissue_group", "temperature_group")
    if (!all(required %in% colnames(sample_map))) stop("Sample map missing: sample_id,tissue_group,temperature_group")
  ...
  map_clean <- sample_map |> filter(sample_id %in% sample_cols) |> distinct(sample_id, .keep_all = TRUE)
  if (nrow(map_clean) == 0) stop("No overlapping samples between sample map and expression columns.")
  ...
  if (!is.na(symbol_col)) {
    symbols <- expr |> select(all_of(c(probe_col, symbol_col))) |> distinct() |> rename(gene_symbol = !!sym(symbol_col))
  } else if (!is.null(opt$`platform-annotation`)) {
    annot <- read_tabular(opt$`platform-annotation`)
    if (!all(c("probe_id", "gene_symbol") %in% colnames(annot))) stop("Platform annotation needs probe_id,gene_symbol")
    symbols <- annot |> select(probe_id, gene_symbol) |> rename(!!probe_col := probe_id)
  } else {
    stop("No gene symbol column found and no --platform-annotation provided.")
  }

  merged <- expr_long |> left_join(symbols, by = probe_col) |> inner_join(map_clean, by = "sample_id")
  ...
  if (nrow(bat) == 0) stop("No BAT samples with COLD/CONTROL labels. Check sample-map inference.")

  bat_samples <- bat |> distinct(sample_id, temperature_group) |> arrange(temperature_group, sample_id)
  ...
  cldn1 <- bat |> filter(safe_upper(gene_symbol) == "CLDN1")
  if (nrow(cldn1) == 0) stop("CLDN1 not found. Check gene symbol mapping.")

  cldn1_vals <- cldn1 |> select(all_of(c(probe_col, "gene_symbol", "sample_id", "temperature_group", "expression")))
  write_csv(cldn1_vals, file.path(out_dir, "csv", "cldn1_sample_values.csv"))

  cldn1_summary <- cldn1 |>
    group_by(temperature_group) |>
    summarise(count = sum(!is.na(expression)), mean = mean(expression, na.rm = TRUE),
              median = median(expression, na.rm = TRUE), sd = sd(expression, na.rm = TRUE), .groups = "drop")
  write_csv(cldn1_summary, file.path(out_dir, "csv", "cldn1_group_summary.csv"))

  cold_vals <- cldn1 |> filter(safe_upper(temperature_group) == "COLD") |> pull(expression) |> na.omit()
  ctrl_vals <- cldn1 |> filter(safe_upper(temperature_group) == "CONTROL") |> pull(expression) |> na.omit()
  ...
  if (length(cold_vals) >= 2 && length(ctrl_vals) >= 2) {
    ...
    t_stat <- NA_real_
    p_value <- NA_real_
    log_lines <- c(log_lines, "WARNING: <2 values in one group for CLDN1; Welch t-test not computed.")
  }

  cldn1_stats <- tibble(
    cold_n = length(cold_vals), control_n = length(ctrl_vals),
    ...
    welch_t_stat = t_stat, p_value = p_value
  )
  ...
  all_gene_stats <- bat |>
    ...
    group_modify(~{
      ...
      tibble(cold_mean = mean(v_cold), control_mean = mean(v_ctrl),
             log2_fc_cold_minus_control = mean(v_cold) - mean(v_ctrl),
             welch_t_stat = unname(t_res$statistic), p_value = t_res$p.value)
    }) |> ungroup()

  if (nrow(all_gene_stats) > 0) {
    all_gene_stats <- all_gene_stats |> mutate(fdr_bh = p.adjust(p_value, method = "BH")) |> arrange(p_value)
  }
  write_csv(all_gene_stats, file.path(out_dir, "csv", "bat_cold_vs_control_all_genes.csv"))

  log_lines <- run_limma(bat = bat, out_dir = out_dir, log_lines = log_lines)

  log_lines <- c(log_lines, "Output files written under:", normalizePath(out_dir))
  writeLines(log_lines, file.path(out_dir, "logs", "run_summary.txt"))
  message("Analysis complete. See run summary: ", file.path(out_dir, "logs", "run_summary.txt"))
